...
class Bot {
  ...
  attachListeners() {
    ...
    this.ircClient.on('kick', (channel, nick, by, reason, message) => {
      _winston2.default.debug('Received kick:', nick, channels);
      this.sendExactToDiscord(channel,'test')
      if (!this.ircStatusNotices || nick === this.ircClient.nick) return;
      channels.forEach(channelName => {
        const channel = channelName.toLowerCase();
        if (!this.channelUsers[channel]) {
          _winston2.default.warn(`No channelUsers found for ${channel} when ${nick} kicked, ignoring.`);
          return;
        }
        if (!this.channelUsers[channel].delete(nick)) return;
        this.sendExactToDiscord(channel, `*${nick}* has been kicked by *${by}* (${reason})`);
      });
    });
